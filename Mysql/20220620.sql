...
/*
    ...
    foreign key : 외래키. join(테이블과 테이블을 연결). null을 허용
					외래키로 지정된 컬럼은 연결된 테이블에서 PK 나 UK 로 (중복X인 키들)걸정되어 있어야 한다.
    check : 범위를 지정. 값을 지정. 지정된 값 외에 사용할 수 없다. null 을 허용
...
-- '' != null 이라서 mysql 에서는 허용, oracle 에서는 안됨
insert into tb_test06(pk_col, col1, col2)
values('', '111', 'aaa');
...
insert into tb_emp(empno, ename, department_id)
values(103, '정수동', '40');	-- '40' 에 해당되는 column 없음 -> error

insert into tb_emp(empno, ename)
values(104, '홍두께');		-- 부서가 없다 null 로 들어감
...
-- check 
-- 지정된 값만 넣을 수 있다. null 을 허용. 중복된 값을 당연히 허용
